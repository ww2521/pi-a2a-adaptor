...
beforeAll(async () => {
    client = new A2AClient({ timeout: 10000, retryAttempts: 0, retryDelay: 0, maxConcurrentTasks: 10, streamingEnabled: true }, { defaultScheme: "none", verifySsl: true });
...
describe("G2: message/send（同步）", () => {
    ...
    it("[14] artifact 文本正确", async () => {
        ...
        expect(text.text).toContain("sync test");
    });
});
// ═══════════════════════════════════════════
...
describe("G9: message/stream (SSE)", () => {
    ...
    it("[38] SSE 事件数 >= 5", async () => {
        const updates = [];
        const task = await client.sendStreamingMessage(agent, userMsg("stream test"), (u) => updates.push(u), { timeout: 15000 });
...
describe("G13: 长任务轮询 + 超时", () => {
    ...
    it("[58] delay:30 轮询超时抛 A2AError(-32007)", async () => {
        await expect(client.sendMessage(agent, userMsg("delay:30", undefined, "ctx-long"), {
    ...
    it("[60] 客户端抛 A2AError(-32007, 'Task exceeded max attempts')", async () => {
        await expect(client.sendMessage(agent, userMsg("delay:30", undefined, "ctx-long2"), {
